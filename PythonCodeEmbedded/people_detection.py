...
class PeopleDetection:  # class for people detection
    ...
    def hsvThresh(self, image):  # thresholding function
        ...
        for i in range(len(output)): # loop through rows
            for j in range(len(output[i])): # loop through columns
                if output[i, j, 0] > 0 or output[i, j, 1] > 0 or output[i, j, 2] > 0: # if pixel is not black
                    output[i, j] = [255, 255, 255] # set pixel to white

        output = self.bitwiseOperation(output, self.heated_object_picture) # bitwise and with heated object picture
        self.thresh_callback(output) # call threshold callback function

    def bitwiseOperation(self, input, input2): # bitwise and function
        img2gray = cv.cvtColor(input2, cv.COLOR_BGR2GRAY) # convert to grayscale
        ret, mask = cv.threshold(img2gray, 10, 255, cv.THRESH_BINARY) # threshold
    ...
    def thresh_callback(self, image): # threshold callback function
        ...
        self.src_gray = cv.cvtColor(src, cv.COLOR_BGR2GRAY) # convert to grayscale
        ...
        kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (kernel, kernel)) # get structuring element
        ...
        for i in range(len(contours)): # loop through contours
            ...
            if area >= areaThresh and length >= lengthThresh: # if area and length are bigger than threshold
                ...
                contoursfixed.append(contours[i]) # add contour to contoursfixed
        # Get the moments
        BiggestContour = [] # biggest contour
        BiggestContour.append(contoursfixed[areas.index(max(areas))]) # add biggest contour to biggest contour
        ...
        for i in range(len(contours)): # loop through contours
            ...
            cv.drawContours(People_mask, contours, i, color, 1) # draw contours
            cv.fillPoly(People_mask, pts=contours, color=(255, 255, 255)) # fill polygon
        # Cropping image to get rid of previously added black borders

        img2gray = cv.cvtColor(People_mask, cv.COLOR_BGR2GRAY) # convert to grayscale
        ret, mask = cv.threshold(img2gray, 10, 255, cv.THRESH_BINARY) # threshold
        ...
        if self.last_frame is not None: # if last frame is not none
            ...
            if diff > threshold:  # moving 
                ...
                if current_time - self.start_time > time_not_moving: # if current time minus start time is greater than time not moving
    ...
    def templateMatchingLogic(self, template, img_rgb): # template matching logic
        ...
        template = cv.cvtColor(template, cv.COLOR_BGR2GRAY) # convert to grayscale
        w, h = template.shape[::-1] # get shape
        res = cv.matchTemplate(img_gray, template, cv.TM_CCOEFF_NORMED) # match template
        ...
        for x in range(24): # loop through 24
            for y in range(32): # loop through 32
                if img_rgb[x][y][0] == 0 and img_rgb[x][y][1] == 0 and img_rgb[x][y][2] == 255: # if red
                    self.coords = [(pt[0], pt[1]), (pt[0] + w, pt[1] + h)] # coords
